                ...
                totalPages.length && totalPages.map(p => {
                    return (
                        <div key={p} className="text-white ">
                            <a className='mt-28 bg-stone-900/75 rounded px-4 py-1 w-6 mx-1 flex justify-center text-emerald-500 text-2xl font-semi-bold font-Poppins' onClick={() => paginado(p)}>{p}</a>                            
                        </div>
                    )
                })
            }
